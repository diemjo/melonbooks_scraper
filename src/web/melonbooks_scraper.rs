...
impl MelonbooksScraper {
    ...
    fn grid_parse_url(node: Node) -> Result<String> {
        let a = node.find(Class("thumb").child(Name("a"))).next().ok_or(HtmlParseError("product_list".to_string()))?;
        let rel_url = a.attr("href").ok_or(HtmlParseError("product_list".to_string()))?;
    ...
    fn parse_title(node: Node) -> Result<String> {
        let str = node.find(Class("bcs").descendant(Name("span"))).last().ok_or(HtmlParseError("product_title".to_string()))?;
    ...
    fn parse_img_url(node: Node) -> Result<String> {
        let img_res = node.find(Class("image").descendant(Name("img"))).next();
        let img = match img_res {
            Some(i) => Ok(i),
            None => node.find(Class("thumb").descendant(Name("img"))).next().ok_or(HtmlParseError("img_url".to_string()))
        }?;
        let img_url = img.attr("src").ok_or(HtmlParseError("img_url".to_string()))?.replace("//", "https://");
        Ok(img_url)
    }
    ...
    fn parse_availability(node: Node) -> Result<Availability> {
        let tr = node.find(Class("drop_cart").descendant(Name("tr"))).filter(
            |tr| tr.find(Name("th")).next().map_or(String::new(), |n| n.inner_html()) == "在庫"
        ).next().ok_or(HtmlParseError("availability".to_string()))?;
        let availability_str = tr.find(Name("td")).next().ok_or(HtmlParseError("availability".to_string()))?.inner_html();
        let availability = match availability_str.as_str() {
            "-" => Ok(NotAvailable),
            "好評受付中" => Ok(Preorder),
            "残りわずか" => Ok(Available),
            "在庫あり" => Ok(Available),
    ...
    fn parse_product(&self, artist: &str, product_url: &str, html: Document) -> Result<Option<Product>> {
        let main_part = html.find(Class("clm_g").child(Class("clm_c"))).next().ok_or(HtmlParseError("product_main_part".to_string()))?;
        ...
        let product = Product::new(product_url.to_string(), title, artist, img_url, date_added, availability);
        //println!("{}", product);
        Ok(Some(product))
    }
}
...
impl WebScraper for MelonbooksScraper {
    ...
    fn get_urls(&self, artist: &str, also_unavailable: bool) -> Result<Vec<String>> {
        ...
        while pageno>0 {
            ...
            let items = html.find(Class("products_big_thumb").child(Name("div")));
            for node in items {
            ...
            writeln!(stdout, "[Search] Found {} products...", 100*(pageno-1)+items_on_page)?;
...
mod tests {
    ...
    fn test_get() {
        ...
        let urls = ws.get_urls("カントク").unwrap();
